 ...
 * 
 * Under Unix, the system browser is hard-coded to be 'mozilla'. Mozilla must be
 * in your PATH for this to work. This has been tested with the following
	...
	public static void displayURL(String url) {
		...
		try {
			if (windows) {
				...
				/**
				 * Try to find a browser in /usr/bin.
				 */
				String UNIX_PATH = "/usr/bin/konqueror";
				File file = new File(UNIX_PATH);
				boolean browserExists = true;
				if (!file.exists()) {
					// konqueror not found
					UNIX_PATH = "/usr/bin/mozilla";
					file = new File(UNIX_PATH);
					if (!file.exists()) {
						// mozilla not found
						UNIX_PATH = "/usr/bin/netscape";
						file = new File(UNIX_PATH);
						if (!file.exists()) {
							// netscape not found
							System.out
									.println("Sorry, no browser found to display Help. We tried /usr/bin/konqueor, /usr/bin/mozilla and /usr/bin/netscape unsuccessfully.");
							browserExists = false;
						}
					}
				}
